DROP FUNCTION IF EXISTS public.get_daily_report(text, text);

CREATE OR REPLACE FUNCTION public.get_daily_report(p_date_start text DEFAULT NULL::text, p_date_end text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
BEGIN
    ...
    -- Fallback to employee token if still no workspace_id
    IF v_workspace_id IS NULL THEN
